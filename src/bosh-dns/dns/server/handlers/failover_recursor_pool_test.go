...
var _ = Describe("RecursorPool", func() {
	Context(`when recursor selection is "serial"`, func() {
		...
		It("always starts at the beginning", func() {
			...
			pool := NewFailoverRecursorPool([]string{"one", "two", "three"}, config.SerialRecursorSelection, 0, 0, fakeLogger)
			...
			pool = NewFailoverRecursorPool([]string{"bad", "two", "three"}, config.SerialRecursorSelection, 0, 0, fakeLogger)
			err = pool.PerformStrategically(work(recursorCallCount))
			Expect(err).NotTo(HaveOccurred())
			Expect(recursorCallCount).To(Equal(map[string]int{
				"one":   1,
			...
			pool = NewFailoverRecursorPool([]string{"bad", "bad", "three"}, config.SerialRecursorSelection, 0, 0, fakeLogger)
			err = pool.PerformStrategically(work(recursorCallCount))
			Expect(err).NotTo(HaveOccurred())
			Expect(recursorCallCount).To(Equal(map[string]int{
				"one":   1,
		...
		Context("when all recursors fail", func() {
			...
			BeforeEach(func() {
				fakeLogger := &loggerfakes.FakeLogger{}
				pool = NewFailoverRecursorPool([]string{"bad", "bad", "bad"}, config.SerialRecursorSelection, 0, 0, fakeLogger)
			})
	...
	Context(`when recursor selection is "smart"`, func() {
		...
		JustBeforeEach(func() {
			...
			pool = NewFailoverRecursorPool(recursors, config.SmartRecursorSelection, 0, 0, fakeLogger)
		})

		It("returns an error if there are no recursors configured", func() {
			pool = NewFailoverRecursorPool([]string{}, config.SmartRecursorSelection, 0, 0, fakeLogger)
			Expect(pool.PerformStrategically(func(string) error { return nil })).To(HaveOccurred())

			pool = NewFailoverRecursorPool(nil, config.SmartRecursorSelection, 0, 0, fakeLogger)
			Expect(pool.PerformStrategically(func(string) error { return nil })).To(HaveOccurred())
		})
